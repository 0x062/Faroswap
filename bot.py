...
# Kunci Pribadi akan dibaca dari file .env
PRIVATE_KEY = os.getenv("PRIVATE_KEY")

# Pengaturan Jumlah Aksi
# Atur ke 0 jika tidak ingin melakukan aksi ini
JUMLAH_SWAP = 5
JUMLAH_TAMBAH_LP = 3
...
# Nominal untuk Tambah Likuiditas (Add Liquidity Pool)
ADD_LP_AMOUNTS = {
    "WPHRS": 0.001,
    "USDC": 0.01,
    "USDT": 0.01,
...
class Faroswap:
    def __init__(self) -> None:
        ...
        self.tickers = ["WPHRS", "USDC", "USDT", "WETH", "WBTC"] # PHRS & WPHRS dikelola terpisah
        
        # ABI Kontrak (Bagian ini tidak perlu diubah)
    ...
    async def perform_swap(self, account, from_ticker, to_ticker, amount_decimal):
        ...
        try:
            ...
            swap_tx['gas'] = int(gas_estimate * 1.5) # Gas buffer
        except Exception as e:
    ...
    async def run(self):
        ...
        if JUMLAH_SWAP > 0:
            ...
            for i in range(JUMLAH_SWAP):
                ...
                # Pilih pasangan token secara acak untuk di-swap
                # Logika: Swap dari token yang dimiliki ke token lain
        ...
        # --- (Bagian Tambah LP bisa ditambahkan di sini dengan logika serupa jika diperlukan) ---
        # Untuk saat ini, saya hanya menyertakan logika SWAP yang paling umum digunakan
        # Jika Anda butuh logika Add LP, saya bisa menambahkannya.

        self.log(f"{Style.BRIGHT}{Fore.GREEN}Semua tugas telah selesai untuk akun {address}.")
